...
(function($){
	...
	function soundicon(currentDeg){

		var volume = (currentDeg / 360) * 10;
		volume = Math.floor(volume);
		volume = volume/10;
		document.getElementById('radio_play').volume = volume ;

		if(currentDeg == 0){
			$( ".sound-icon" ).remove();
			$("#bars").append("<span class='sound-icon fadeOut'><i class='fa fa-volume-off'></i></span>");
		}else if(currentDeg < 200){
			$( ".sound-icon" ).remove();
			$("#bars").append("<span class='sound-icon fadeOut'><i class='fa fa-volume-down'></i></span>");
		}else {
			$( ".sound-icon" ).remove();
			$("#bars").append("<span class='sound-icon fadeOut'><i class='fa fa-volume-up'></i></i></span>");
		}
	}
})(jQuery);
